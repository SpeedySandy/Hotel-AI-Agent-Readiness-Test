...
function doPost(e) {
  try {
    ...
    sheet.appendRow([
      ...
      data.email||'', data.name||'', data.company||'', data.role||'',
      data.hotelUrl||'', data.score||'', data.grade||'', data.source||'',
    ]);
...
function doGet(e) {
  ...
  if (pwd !== DASHBOARD_PASSWORD) return respond(cb, { error: 'unauthorized' });
  try {
    const sheet  = getOrCreateSheet();
